...
def main(args):
    ...
    for i in range(args.per_device_train_batch_size):
        idx_vec.append(torch.cat((2*torch.ones(int(args.num_pos),1),3*torch.ones(args.num_neg,1), 4*torch.ones(args.num_rnd,1)),dim=0))
    ...
    class CustomTrainer(transformers.Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            if True:
                ...
                NUM_OFFSET =  args.num_pos +  args.num_neg + args.num_rnd
                NUM_POS = args.num_pos*1 # must be dividble by 2
                #NUM_SAMPLES = args.num_pos + 1 # + reference
                NUM_NEG =  args.num_neg*1# must be dividible by 2
                NUM_RAND_NEG = args.num_rnd#int(args.num_pos/2)
                ...
                final_scores = torch.cat([torch.exp(pos_scores_), torch.exp( neg_scores_), torch.exp( rnd_scores_)], 1)/self.tau
                label_probs_ = torch.cat([torch.ones_like(pos_scores_), torch.zeros_like(neg_scores_), torch.zeros_like(rnd_scores_)],1).to(final_scores.device)
                
                weights = torch.cat([torch.zeros_like(pos_scores_), self.alpha*torch.ones_like(neg_scores_), torch.zeros_like(rnd_scores_)],1).to(final_scores.device)
